...
int find_sym(const vector<match>& m, int size)
{
	vector<match> e;
	for (auto it = m.begin(); it != m.end() && it->d == 0; ++it)
		e.push_back(*it);
	r::sort(e, [](auto& e1, auto& e2) { return e1.r.second - e1.r.first < e2.r.second - e2.r.first; });
	if ((e.size() == size || e.size() == size - 1) && e[0].r.second - e[0].r.first == 1 && (e.back().r.first == 0 || e.back().r.second == size-1))
	auto it = find_if(e.begin(), e.end(), [](auto& m) {return m.r.first == m.r.second + 1; });
	bool matched = true;
	return 0;
}
